import 'package:bloc_docs_tutorials/bloc_tutorials/flutter_timer/ticker.dart';
...
void main() {
  group('TimerBloc', () {
    ...
    test('initial state is TimerInitial(60)', () {
      expect(TimerBloc(ticker: ticker).state, TimerInitial(60));
    });
    ...
    blocTest<TimerBloc, TimerState>(
      ...
      setUp: () {
        when(
          () => ticker.tick(ticks: 3),
        ).thenAnswer((_) => Stream<int>.value(3));
      },
      build: () => TimerBloc(ticker: ticker),
    ...
    blocTest<TimerBloc, TimerState>(
      ...
      setUp: () {
        when(
          () => ticker.tick(ticks: 1),
        ).thenAnswer((_) => Stream<int>.fromIterable([1, 0]));
      },
      build: () => TimerBloc(ticker: ticker),
